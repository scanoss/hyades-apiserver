...
public class CryptoMaterialCelPolicyScriptSourceBuilder implements CelPolicyScriptSourceBuilder {
    ...
    public String apply(final PolicyCondition policyCondition) {
        ...
        return switch (policyCondition.getOperator()) {
            case IS -> """
                    crypto_asset.related_material.type == "%s"
                    """.formatted(value);
            case IS_NOT -> """
                    crypto_asset.related_material.type != "%s"
                    """.formatted(value);
            case MATCHES -> """
                    crypto_asset.related_material.type.matches("%s")
                    """.formatted(value);
            default -> null;
        };
    }
