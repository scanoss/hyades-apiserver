...
public class CryptoProtocolCelPolicyScriptSourceBuilder implements CelPolicyScriptSourceBuilder {
    ...
    public String apply(final PolicyCondition policyCondition) {
        ...
        return switch (policyCondition.getOperator()) {
            case IS -> """
                    crypto_asset.protocol.protocol_version == "%s"
                    """.formatted(value);
            case IS_NOT -> """
                    crypto_asset.protocol.protocol_version != "%s"
                    """.formatted(value);
            case MATCHES -> """
                    crypto_asset.protocol.protocol_version.matches("%s")
                    """.formatted(value);
            default -> null;
        };
    }
